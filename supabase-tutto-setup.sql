...
-- ---------------------------------------------------------
-- 81) Genera Coupon: form per creare codici sconto scritti su
-- WooCommerce. Base anche per la futura generazione automatica dei
-- premi alle master. "valido_da" non esiste in WooCommerce (che ha
-- solo la scadenza): un coupon con valido_da futura resta in stato
...
-- ---------------------------------------------------------
-- 82) Gestione Hotel diventa una pagina completa (prima solo nome, in
-- un modale): ogni hotel prevede telefono, email, indirizzo, civico,
...
-- ---------------------------------------------------------
-- 83) Assegnazione Master: dopo Alloggio, dettagli della prenotazione
-- per ogni persona (master principale e righe extra master/
-- assistente/leva) — richiesta fattura, notti prenotate, importo
-- pattuito a notte e per l'intero periodo, pagato.
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 84) Riepilogo amministrativo di una classe: ogni spesa può avere una
...
-- ---------------------------------------------------------
-- 85) Strutture compensi riutilizzabili: una struttura (nome + fasce
...
-- ---------------------------------------------------------
-- 91) Costo giornaliero di una sede, in due varianti a seconda del
...
-- ---------------------------------------------------------
-- 93) Gestione Hotel: percentuale IVA da scorporare dal "Costo a
-- notte — Fattura" (0/4/10/22%).
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 94) Riepilogo amministrativo: split libero Bonifico/Cash per ogni
-- voce (compenso master, costo location), non più un flag/tendina
...
-- ---------------------------------------------------------
-- 96) Ogni "fonte" di costo di un corso (assistente, master, sede,
...
-- ---------------------------------------------------------
-- 97) Riepilogo amministrativo: quando una classe incassa quote
-- modelle, il 50% va riportato come costo "Commissione ricerca
-- modelle" (pagato in cash di default). Split libero Bonifico/Cash
-- come le altre righe automatiche della tabella spese.
-- ---------------------------------------------------------
insert into public.costi_sottocategorie (id, categoria_id, nome, ordine, automatico, campo_automatico) values
...
-- ---------------------------------------------------------
-- 98) Gestione categorie prodotti: una categoria può essere esclusa
-- dalla vendita diretta (POS/online) — i suoi prodotti restano nel
...
-- ---------------------------------------------------------
-- 99) "Associa il gruppo a una categoria di spesa": una sola categoria
...
-- ---------------------------------------------------------
-- 100) "Bonifico Fattura" (spunta informativa, mai usata nei calcoli)
...
-- ---------------------------------------------------------
-- 101) Scadenziario: elenco unico dei bonifici da pagare su tutti i
-- corsi conclusi, con archivio "Evase".
--
...
-- ---------------------------------------------------------
-- 102) "Fissa vista su tutti i terminali" in Assegnazione Master: chi
...
-- ---------------------------------------------------------
-- 103) Maniglie trascinabili (stile "ridimensiona colonna" di Excel) per
...
-- ---------------------------------------------------------
-- 105) Scadenza pagamento per Alloggio e Location: un bonifico prenotato
...
-- ---------------------------------------------------------
-- 106) Scadenza pagamento sulla spesa vera: quando una fattura arriva
...
-- ---------------------------------------------------------
-- 107) "Gestisci alloggio": il pattuito a notte diventa due campi
-- affiancati, Cash e Bonifico, invece di uno solo che cambiava
-- significato secondo la tendina "Tipo di pagamento" — così si tengono
-- entrambe le tariffe negoziate con l'hotel senza doverle riscrivere ogni
-- volta che si cambia tipo di pagamento.
-- ---------------------------------------------------------
do $$
begin
  if exists (
...
-- ---------------------------------------------------------
-- 109) Location → "Sedi esistenti": nome della sede oltre alla città (i
...
-- ---------------------------------------------------------
-- 110) "Aggiungi location" falliva su una seconda sede nella stessa città
...
-- ---------------------------------------------------------
-- 111) Elenco "Città" (Impostazioni → Location, subito dopo "Associa il
...
-- ---------------------------------------------------------
-- 112) IBAN su master, location e fornitori: per lo Scadenziario Passivo,
-- che ora elenca ogni fattura da pagare (data del debito, scadenza,
...
-- ---------------------------------------------------------
-- 113) Dati fiscali su master e assistenti: la stessa "appendice" già
-- presente sulla scheda hotel (indirizzo, partita IVA, codice
-- destinatario, PEC, IBAN) — l'assistente non aveva ancora nemmeno
-- l'IBAN, il master aveva già l'IBAN ma non il resto. Compilazione
-- manuale, nessun valore di default.
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 114) Classificazione gestionale + Budget e controllo su location,
-- master e assistente: gli stessi campi già presenti sulle spese
-- (PaginaSpesaForm) — Diretto/Indiretto, Fisso/Variabile, Ricorrente/
-- Occasionale, Natura, Controllabilità, Riducibilità, Essenzialità,
-- Origine, Ricorrenza, Bene durevole, Incluso nell'analisi dei costi,
-- Budget previsto, Soglia di allerta personalizzata, Responsabile del
-- costo. Compilazione manuale, solo informativi: non vengono (ancora)
-- riportati in automatico sulle spese generate da queste anagrafiche.
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 115) Ripristina categoria_spesa_id su location: era stata aggiunta il
...
-- ---------------------------------------------------------
-- 116) Classificazione gestionale + Budget e controllo anche su hotel:
-- lo stesso blocco già aggiunto a location, master e assistente
-- (sezione 114), inizialmente escluso da hotel su richiesta esplicita
-- e ora richiesto anche qui, con le stesse funzioni (pannello a
-- tendina, flag "applica a tutti").
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 117) "Abbonamenti e contratti": nuovo tab di Contabilità, dopo Scadenziario
...
-- ---------------------------------------------------------
-- 118) Integrazione Fatture in Cloud: legge le fatture di acquisto
...
-- ---------------------------------------------------------
-- 119) Riconciliazione fatture passive — sezioni 4 (Modello dati) e 5
...
-- 123) Riconciliazione fatture: campo note su documento_fornitore
-- ---------------------------------------------------------
...
-- 124) Anagrafiche: telefono/email su fornitori
-- ---------------------------------------------------------
...
-- 125) Anagrafiche: categoria/sottocategoria di default sui fornitori
-- ---------------------------------------------------------
...
-- 126) Gestione Location: dati di contatto/fiscali sulla sede
-- ---------------------------------------------------------
-- Gestione Location: stessi dati di contatto/fiscali del fornitore
-- sulla scheda sede (telefono, email, indirizzo, P.IVA, codice
-- fiscale) — restano in più i campi propri della sede (città,
-- capienza, costo giornaliero cash/bonifico).
--
-- categoria_spesa_id (già presente, un solo select combinato
-- categoria+sottocategoria) diventa sottocategoria_id, affiancata da
-- una nuova categoria_id: stesso schema a due colonne già usato su
-- fornitori, stessa UI a due tendine dipendenti. Rename, non
-- ricreazione: i valori esistenti (tutti validi, verificato) restano.
--
-- Additivo per le nuove colonne, nessuna riga toccata a parte il nome
-- della colonna rinominata.
-- ---------------------------------------------------------
...
-- 127) Anagrafiche: collegamento esplicito fornitore -> location/hotel
-- ---------------------------------------------------------
...
-- 128) Logistica prodotti: "Gestione rientro"
-- ---------------------------------------------------------
...
-- 134) "Trasmetti inventario" (Inventario Master → Logistica prodotti)
-- ---------------------------------------------------------
...
-- 135) "Segnalazioni" (Inventario Master → Magazzini esterni)
-- ---------------------------------------------------------
...
-- 136) "Prodotti sciolti" — terza voce di "Prodotti che sto per rispedire"
-- ---------------------------------------------------------
...
-- 137) Referral code per master — "Genera Coupon" a 3 tab
-- ---------------------------------------------------------
